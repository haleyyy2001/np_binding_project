...
for num_tcr = 100:10:1000
    dens_tcr = [];
    dens_cluster = [];
    prob = [];
    
    for tcr_per_cluster = 1:0.1:50
        num_clusters = (num_tcr/tcr_per_cluster);
        dens_tcr(end+1) = tcr_per_cluster / (pi * cluster_radius^2);
        dens_cluster(end+1) = num_clusters / (pi * rSurf^2);
        ...
        prob(end+1) = bind_freq;
    end
    
    plot3(dens_tcr, dens_cluster, prob, 'LineWidth', 1.5); % Added linewidth for better visibility
end

% Improve figure appearance
xlabel('Density of TCR', 'FontSize', 12);
ylabel('Density of Cluster', 'FontSize', 12);
zlabel('Binding Frequency', 'FontSize', 12);
title('TCR and Cluster Density vs. Binding Frequency', 'FontSize', 14);
grid on
colorbar
hold off
