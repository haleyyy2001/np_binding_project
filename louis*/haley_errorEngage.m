        ...
        if ansr == 'y';
            ...
            for i = 1:length(rs)
                ...
                for j=1:length(vs)
                    ...
                    figure(i)
                    semilogx(vs(j)*datav(:,3),datav(:,4),':o','color',cols(j,:),'linewidth',1.5, 'DisplayName', txt1)
                    hold on
                    semilogx(vs(j)*datav(:,3),datav(:,5),'-s','color',0.8*cols(j,:),'linewidth',1.5, 'DisplayName', txt2)
                    %{
                    figure(i)
                ...
                xlabel('pMHC Concentration (\times 10^{11})')
