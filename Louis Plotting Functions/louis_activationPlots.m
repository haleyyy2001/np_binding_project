...
chain = load([pp,'fig_nsim1500date14-Feb-2023/chain.txt']);
sschain = load([pp,'fig_nsim1500date14-Feb-2023/sschain.txt']);
...
for i = 1:28%length(data(:,1))
    ...
    y_exp = [y_exp, data(i,4)*data(i,2)];
 
    y2 = louis_activationEngage(kd,KD,theta_deg,np_params,ifng_params,pho_params,pp);
    y_pred = [y_pred, y2*v];
