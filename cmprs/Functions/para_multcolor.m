...
% ... [previous code to create data_table] ...
...
% Calculate colors for each line (same as before)
lineColors = arrayfun(getColor, data_table.bind_probability, 'UniformOutput', false);

% Convert the table to a matrix for parallelcoords
data_matrix = table2array(data_table);

% Create a parallel coordinates plot
figure;
parallelcoords(data_matrix, 'Group', data_table.bind_probability, 'Labels', data_table.Properties.VariableNames);

% Customize the plot if needed
% [Add your customization code here, if necessary]

% Note: Parallelcoords does not support the same level of customization
% as parallelplot, especially for individual line colors.
% You may need to adjust the above code to fit your specific needs.
